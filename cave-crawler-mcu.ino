...
void processIMU()
{
  ...
  unsigned long diff=micros() - packet.timestamp_us;
  if(diff > max_diff)
    max_diff=diff;
  ...
  em7180.checkEventStatus(); // also clears the interrupt
  ...
  {
    em7180.readQuaternion(packet.qw, packet.qx, packet.qy, packet.qz);  
